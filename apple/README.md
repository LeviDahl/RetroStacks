...
| Regions | ✅ catalog items carry `regions`; North America only by default, a per-screen "non-NA regions" toggle (`system.showNonNARegions`) shows EU/JP. All 19 platforms carry region data, live in Supabase since the 2026-09-19 migrate run (verified: ~97-100% of rows per platform) |
